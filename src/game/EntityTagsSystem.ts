...
export class EntityTagsSystem {
  ...
  getEntityTags() {
    ...
    const projectEntity = (
      ...
      this._tagToEntity
        .subVectors(this._tagTempVec, this.game.camera.position)
        .normalize();
    ...
    const isDungeonDelver = useGameStore
      .getState()
      .currentMode.startsWith("dungeondelver");
    ...
    if (isDungeonDelver) {
      ...
      if (localPlayer && localPlayer.perspective !== Perspective.FIRST_PERSON) {
        const combatLevel =
          useGameStore.getState().playerSkills?.Combat?.level || 1;
        const heightOffset = localPlayer.inputController?.isCrouching
          ? 1.8
          : 2.2;
        ...
        localPlayerFeetPos.y =
          localPlayer.worldPosition.y -
          localPlayer.playerHeight +
          (localPlayer.cameraYOffset || 0);

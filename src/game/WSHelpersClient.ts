...
export function encodePacketClient(
  event: string,
  args: any[],
): string | ArrayBuffer {
  ...
  if (
    args.length === 1 &&
    (args[0] instanceof ArrayBuffer || ArrayBuffer.isView(args[0]))
  ) {
    const eventBuf = new TextEncoder().encode(event);
    const argBuf = args[0] instanceof ArrayBuffer ? args[0] : args[0].buffer;
    const argView = new Uint8Array(
      argBuf,
      (args[0] as ArrayBufferView).byteOffset || 0,
      (args[0] as ArrayBufferView).byteLength || argBuf.byteLength,
    );
...
export function decodePacketClient(
  data: string | ArrayBuffer | Blob,
):
  | Promise<{ event: string; args: any[] } | null>
  ...
  if (typeof data === "string") {
    ...
    return new Promise((resolve) => {
      ...
      reader.onload = () =>
        resolve(decodePacketClient(reader.result as ArrayBuffer) as any);

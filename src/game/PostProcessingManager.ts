...
export class PostProcessingManager {
  ...
  constructor(game: Game) {
    ...
    if (
      !disableComposer &&
      (this.game.world.isSummerLab || usePremiumShaders)
    ) {
  ...
  dispose() {
    if (this.composer) {
      ...
      if ((this.composer as any).renderTarget1)
        (this.composer as any).renderTarget1.dispose();
      if ((this.composer as any).renderTarget2)
        (this.composer as any).renderTarget2.dispose();
    }
  }
}

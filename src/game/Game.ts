...
export class Game {
  ...
  private getResolvedDpr(perfMode: boolean) {
    ...
    if (perfMode) {
      ...
      return isMobile
        ? Math.min(0.8, window.devicePixelRatio)
        : Math.min(0.8, window.devicePixelRatio);
    } else {
      return isMobile
        ? Math.min(1.0, window.devicePixelRatio)
        : Math.min(1.0, window.devicePixelRatio); // Let desktop use standard, mobile 1.0
    }
  }
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.renderer.setPixelRatio(
      this.getResolvedDpr(initialSettings.performanceMode),
    );
    this.renderer.setSize(window.innerWidth, window.innerHeight);
    const effectivePremiumShaders =
      initialSettings.premiumShaders && !initialSettings.performanceMode;
    ...
    this.settingsUnsubscribe = settingsManager.subscribe(
  ...
  applySettings(settings: GameSettings) {
    ...
    const effectivePremiumShaders =
      settings.premiumShaders && !settings.performanceMode;
    if (this.lastPremiumShaders !== effectivePremiumShaders) {
      ...
      const dirLight = this.scene.getObjectByName(
        "sun",
    ...
    if (this.lastPerformanceMode !== settings.performanceMode) {
      ...
      this.renderer.setPixelRatio(
        this.getResolvedDpr(settings.performanceMode),
      );
      this.renderer.setSize(window.innerWidth, window.innerHeight);
    }
  }
  ...
  stop() {
    ...
    if (this.world.transparentMaterial)
      this.world.transparentMaterial.dispose();
  ...
  loop = (isBackgroundTick?: boolean | number | Event) => {
    ...
    // Check loading state
    if (
      useGameStore.getState().isMapLoading &&
      this.player.hasReceivedInitialRespawn
    ) {
      ...
      for (let x = -radius; x <= radius; x++) {
        for (let z = -radius; z <= radius; z++) {
          ...
          if (chunk) {
            loadedCount++;
            if (chunk.mesh || (!chunk.needsUpdate && !chunk.isMeshing))
              meshedCount++;
          }
        }
      }
      ...
      if (loadedCount < TOTAL_CHUNKS)
        msg = `Generating Terrain (${loadedCount}/${TOTAL_CHUNKS})`;
      else if (meshedCount < TOTAL_CHUNKS)
        msg = `Building Geometry (${meshedCount}/${TOTAL_CHUNKS})`;
      else msg = "Spawning Entities";

      const now = performance.now();
      if (
        !(this as any)._lastLoadUpdate ||
        now - (this as any)._lastLoadUpdate > 100 ||
        progress >= 1
      ) {
    ...
    while (this.world.meshesToRemove.length > 0) {
      ...
      if (disposing.transparentMesh)
        disposing.transparentMesh.geometry.dispose();
    }
    ...
    if ((this.world.transparentMaterial as any).userData?.uTime) {
      (this.world.transparentMaterial as any).userData.uTime.value =

...
export class ChocolateFluidSystem {
  ...
  splatGrid = new Map<
    string,
    { idx: number; timestamp: number; colorHex: number }
  >();
  splatKeys: string[] = [];
  ...
  initProjectiles() {
    ...
    this.projectileMesh.instanceColor = new THREE.InstancedBufferAttribute(
      new Float32Array(this.maxProjectiles * 3),
      3,
    );
    this.projectileMesh.instanceColor.setUsage(THREE.DynamicDrawUsage);

    const alphaAttrib = new THREE.InstancedBufferAttribute(
      new Float32Array(this.maxProjectiles).fill(1.0),
      1,
    );
  ...
  initSplats() {
    ...
    this.splatMesh.instanceColor = new THREE.InstancedBufferAttribute(
      new Float32Array(this.maxSplats * 3),
      3,
    );
    ...
    this.squareSplatMesh = new THREE.InstancedMesh(
      geo,
      squareMat,
      this.maxSplats,
    );
    this.squareSplatMesh.instanceMatrix.setUsage(THREE.DynamicDrawUsage);
    this.squareSplatMesh.instanceColor = new THREE.InstancedBufferAttribute(
      new Float32Array(this.maxSplats * 3),
      3,
    );
  ...
  update(delta: number) {
    ...
    for (const req of this.emitRequests) {
      ...
      const toSpawnRand = seededRandom(
        originSeedX,
        originSeedY,
        originSeedZ,
        0,
      ...
      const toSpawn =
        Math.floor(spawnRate * delta) +
        (toSpawnRand < (spawnRate * delta) % 1 ? 1 : 0);

      for (let i = 0; i < toSpawn; i++) {
        // Interpolate to create a continuous unbroken line across frames
        const t = toSpawn > 1 ? i / (toSpawn - 1) : 1;
        const interpOrigin = new THREE.Vector3().lerpVectors(
          lastOrigin,
          req.origin,
          t,
        );
        ...
        const rand4 = seededRandom(
          interpOrigin.x,
          interpOrigin.y,
          interpOrigin.z,
          randSeedBase + 3,
        );
        const rand5 = seededRandom(
          interpOrigin.x,
          interpOrigin.y,
          interpOrigin.z,
          randSeedBase + 4,
        ...
        const spawnThickness = req.isSpray
          ? 0.4 + rand1 * 0.4
          : THREE.MathUtils.lerp(0.4, 1.45, pumpAlpha);
        ...
        if (idx !== -1) {
          ...
          this.projectileMesh.geometry.attributes.instanceAlpha.setX(
            idx,
            alpha,
          );
          ...
          this.projectilePositions[idx]
            .copy(interpOrigin)
            .add(
              new THREE.Vector3(
                (rand3 - 0.5) * originSpreadAmount,
                (rand4 - 0.5) * originSpreadAmount,
                (rand5 - 0.5) * originSpreadAmount,
              ),
            );

          // Velocity spread
          const spreadAmount = req.isSpray ? 4.0 : 0.1;
          const spreadRand1 = seededRandom(
            interpOrigin.x,
            interpOrigin.y,
            interpOrigin.z,
            randSeedBase + 5,
          );
          const spreadRand2 = seededRandom(
            interpOrigin.x,
            interpOrigin.y,
            interpOrigin.z,
            randSeedBase + 6,
          );
          const spreadRand3 = seededRandom(
            interpOrigin.x,
            interpOrigin.y,
            interpOrigin.z,
            randSeedBase + 7,
          );
          const spreadRand4 = seededRandom(
            interpOrigin.x,
            interpOrigin.y,
            interpOrigin.z,
            randSeedBase + 8,
          ...
          const spread = new THREE.Vector3(
            (spreadRand1 - 0.5) * spreadAmount,
            (spreadRand2 - 0.5) * spreadAmount,
            (spreadRand3 - 0.5) * spreadAmount,
          );

          // Include emitter's base velocity if available
          this.projectileVelocities[idx]
            .copy(interpDir)
            .multiplyScalar(req.isSpray ? 20.0 + spreadRand4 * 8.0 : 26.0)
            .add(spread)
            .add(req.velocity);

          // Precise backdating for sub-frame continuity
          this.projectilePositions[idx].addScaledVector(
            this.projectileVelocities[idx],
            -t * delta,
          );
        ...
        if (!hideNow) {
          ...
          if (dist > 0.0001) {
            ...
            if (hitInfo.hit && hitInfo.hitPoint) {
              ...
              // Spawn a Splat!
              const hitNorm = hitInfo.prevPos
                .clone()
                .sub(hitInfo.blockPos)
                .normalize();
              // Prevent zero normal
          ...
          if (!hideNow) {
            ...
            if (speed > 0.1) {
              this.splatDummy.quaternion.setFromUnitVectors(
            ...
            const baseThickness = Math.min(
              3.0,
              (this.projectileInitialScales[i] || 1.35) + age * 2.0,
            );
    ...
    if (this.projectileUpdated) {
      ...
      if (this.projectileMesh.instanceColor)
        this.projectileMesh.instanceColor.needsUpdate = true;
      if (this.projectileMesh.geometry.attributes.instanceAlpha)
        this.projectileMesh.geometry.attributes.instanceAlpha.needsUpdate = true;
    }
    ...
    if (this.splatUpdated) {
      if (this.maxSplatIdx >= this.minSplatIdx) {
        ...
        (this.splatMesh.instanceMatrix as any).updateRange = {
          offset: offsetMat,
          count: countMat,
        };
        (this.squareSplatMesh.instanceMatrix as any).updateRange = {
          offset: offsetMat,
          count: countMat,
        };
        this.splatMesh.instanceMatrix.needsUpdate = true;
        ...
        if (this.splatMesh.instanceColor) {
          (this.splatMesh.instanceColor as any).updateRange = {
            offset: offsetCol,
            count: countCol,
          };
          this.splatMesh.instanceColor.needsUpdate = true;
        ...
        if (this.squareSplatMesh.instanceColor) {
          (this.squareSplatMesh.instanceColor as any).updateRange = {
            offset: offsetCol,
            count: countCol,
          };
          this.squareSplatMesh.instanceColor.needsUpdate = true;
    ...
    if (this.pendingSplats.length > 0) {
      if (
        !this.lastSplatEmitTime ||
        Date.now() - this.lastSplatEmitTime > 100
      ) {
    ...
    if (this.pendingCleanSplats.length > 0) {
      if (
        !this.lastCleanEmitTime ||
        Date.now() - this.lastCleanEmitTime > 100
      ) {

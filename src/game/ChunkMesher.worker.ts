...
export function runMesher(data: ChunkMesherRequest): ChunkMesherResponse {
  ...
  class DynamicFloat32Buffer {
    ...
    push12(
      ...
      this.data[this.length++] = j;
      this.data[this.length++] = k;
      this.data[this.length++] = l;
    }
    _grow(minAmount: number) {
      const newSize = Math.max(
        this.data.length * 2,
        this.data.length + minAmount,
      );
  ...
  const addFace = (
    ...
    if (isSlab(blockType)) {
      ...
      p[7][1] -= hOffset;
      p[7][2] += m + bevel;
    } else if (blockType === BLOCK.LAUNCHER) {
      // Floor pad
      p[0][0] = x + 0.1;
      p[0][2] = z + 0.9;
      p[1][0] = x + 0.9;
      ...
      p[7][1] = y + 0.1;
      p[7][2] = z + 0.1;
    } else if (blockType === BLOCK.LAUNCHER_WALL_X_POS) {
      // On +X wall
      p[0][0] = x + 0.9;
      p[0][1] = y + 0.1;
      p[0][2] = z + 0.9;
      ...
      p[7][1] = y + 0.9;
      p[7][2] = z + 0.1;
    } else if (blockType === BLOCK.LAUNCHER_WALL_X_NEG) {
      // On -X wall
      p[0][0] = x + 0.0;
      p[0][1] = y + 0.1;
      p[0][2] = z + 0.9;
      ...
      p[7][1] = y + 0.9;
      p[7][2] = z + 0.1;
    } else if (blockType === BLOCK.LAUNCHER_WALL_Z_POS) {
      // On +Z wall
      p[0][0] = x + 0.1;
      p[0][1] = y + 0.1;
      p[0][2] = z + 1.0;
      ...
      p[7][1] = y + 0.9;
      p[7][2] = z + 0.9;
    } else if (blockType === BLOCK.LAUNCHER_WALL_Z_NEG) {
      // On -Z wall
      p[0][0] = x + 0.1;
      p[0][1] = y + 0.1;
      p[0][2] = z + 0.1;
    ...
    if (layer === opaque) {
      switch (dir) {
        case 0:
          ao[0] = getAO(isSolid(1, -1, 0), isSolid(1, 0, 1), isSolid(1, -1, 1));
          ao[1] = getAO(
            isSolid(1, -1, 0),
            isSolid(1, 0, -1),
            isSolid(1, -1, -1),
          );
          ao[2] = getAO(isSolid(1, 1, 0), isSolid(1, 0, -1), isSolid(1, 1, -1));
          ao[3] = getAO(isSolid(1, 1, 0), isSolid(1, 0, 1), isSolid(1, 1, 1));
          break;
        case 1:
          ao[0] = getAO(
            isSolid(-1, -1, 0),
          ...
          break;
        case 2:
          ao[0] = getAO(isSolid(-1, 1, 0), isSolid(0, 1, 1), isSolid(-1, 1, 1));
          ao[1] = getAO(isSolid(1, 1, 0), isSolid(0, 1, 1), isSolid(1, 1, 1));
          ao[2] = getAO(isSolid(1, 1, 0), isSolid(0, 1, -1), isSolid(1, 1, -1));
          ao[3] = getAO(
            isSolid(-1, 1, 0),
            isSolid(0, 1, -1),
            isSolid(-1, 1, -1),
        ...
        case 4:
          ...
          ao[1] = getAO(isSolid(1, 0, 1), isSolid(0, -1, 1), isSolid(1, -1, 1));
          ao[2] = getAO(isSolid(1, 0, 1), isSolid(0, 1, 1), isSolid(1, 1, 1));
          ao[3] = getAO(isSolid(-1, 0, 1), isSolid(0, 1, 1), isSolid(-1, 1, 1));
          break;
        case 5:
          ao[0] = getAO(
            isSolid(1, 0, -1),
    ...
    layer.uvs.push8(0, 0, 1, 0, 1, 1, 0, 1);
  ...
  const addCross = (
    ...
    const hash =
      Math.abs(
        Math.sin(wx * 12.9898 + (y + WORLD_Y_OFFSET) * 78.233 + wz * 37.719) *
          43758.5453,
      ) % 1;
    ...
    const heightBase = isTorch
      ? 0.6
      : blockType === BLOCK.WHEAT
        ? 1.4
    ...
    if (blockType === BLOCK.TORCH_WALL_X_POS) {
      topOffsetX = -0.15;
      botOffsetX = 0.4;
      botOffsetY = 0.2;
    } else if (blockType === BLOCK.TORCH_WALL_X_NEG) {
      topOffsetX = 0.15;
      botOffsetX = -0.4;
      botOffsetY = 0.2;
    } else if (blockType === BLOCK.TORCH_WALL_Z_POS) {
      topOffsetZ = -0.15;
      botOffsetZ = 0.4;
      botOffsetY = 0.2;
    } else if (blockType === BLOCK.TORCH_WALL_Z_NEG) {
      topOffsetZ = 0.15;
      botOffsetZ = -0.4;
      botOffsetY = 0.2;
    }
    ...
    const p = [
      ...
      [
        px + inset + s + botOffsetX,
        y + botOffsetY,
        pz + inset + s + botOffsetZ,
      ],
      [
        px + inset + s + topOffsetX,
        y + height + botOffsetY,
        pz + inset + s + topOffsetZ,
      ],
      [
        px + inset + topOffsetX,
        y + height + botOffsetY,
    ...
    const pushFace = (
      ...
      if (reverse) {
        ...
        layer.tileBases.push8(u, v, u, v, u, v, u, v);
        layer.uvs.push8(0, 1, 1, 1, 1, 0, 0, 0);
      } else {
        layer.positions.push12(
          p0[0],
          p0[1],
      ...
      const lightMult = isTorch
        ? 1.0
        : Math.max(0.35, Math.pow(0.85, 15 - light));
      ...
      for (let z = 0; z < CHUNK_SIZE; z++) {
        ...
        if (x < 15) {
          ...
          const c = {
            ...
            light: data.neighborsLight[2 + 1 * 3],
          };
        ...
        if (
          nType === BLOCK.AIR ||
          (isTransparent(nType) &&
            !(typeIsWater && isWater(nType)) &&
            nType !== type &&
            !(typeIsWater && isGlass(nType))) ||
          (isCutout(nType) && nType !== type) ||
          (!typeIsSlab && isSlab(nType)) ||
          typeIsChest
        ) {
          if (isFullBlock) {
            let ao0 = 3,
        ...
        if (x > 0) {
          ...
          const c = {
            ...
            light: data.neighborsLight[0 + 1 * 3],
          };
          const isCMeshed = !!(c && c.blocks);
          nType = isCMeshed
            ? c.blocks[15 | (z << 4) | (y << 8)]
        ...
        if (z < 15) {
          ...
          const c = {
            ...
            light: data.neighborsLight[1 + 2 * 3],
          };
          const isCMeshed = !!(c && c.blocks);
          nType = isCMeshed
            ? c.blocks[x | (0 << 4) | (y << 8)]
        ...
        if (z > 0) {
          ...
          const c = {
            ...
            light: data.neighborsLight[1 + 0 * 3],
          };
          const isCMeshed = !!(c && c.blocks);
          nType = isCMeshed
            ? c.blocks[x | (15 << 4) | (y << 8)]

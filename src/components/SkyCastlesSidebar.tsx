...
export const SkyCastlesSidebar: React.FC<SkyCastlesSidebarProps> = ({
  isMobile,
}) => {
  ...
  const [syncState, setSyncState] = useState<any>(
    (window as any).latestSkyCastlesSync || null,
  );
  ...
  return (
    ...
    >
      {recentRewards.map((reward) => (
        <div
          key={reward.id}
          className="absolute -left-32 top-11 text-[#FFFF55] font-bold text-lg mc-text-shadow animate-[slideUpFade_2s_ease-out_forwards]"
        >
      ...
      <div className="bg-black/60  p-3 md:p-4 border-l-4 border-[#FFAA00] text-white text-sm md:text-base shadow-2xl min-w-[160px] md:min-w-[200px]">
        ...
        <div className="space-y-2">
          {syncState && (
            <div className="flex flex-col pt-1 pb-2 gap-1.5">
              <div className="text-white text-xs font-bold mc-text-shadow flex justify-between items-center">
                ...
                <span
                  className={
                    syncState.redHp > 0 ? "text-[#FF5555]" : "text-gray-500"
                  }
                >
                  {syncState.redHp > 0
                    ? `${Math.ceil(syncState.redHp)}/${syncState.redMax}`
                    : "DEAD"}
                </span>
              </div>
              ...
              <div className="text-white text-xs font-bold mc-text-shadow flex justify-between items-center mt-1">
                ...
                <span
                  className={
                    syncState.blueHp > 0 ? "text-[#5555FF]" : "text-gray-500"
                  }
                >
                  {syncState.blueHp > 0
                    ? `${Math.ceil(syncState.blueHp)}/${syncState.blueMax}`
                    : "DEAD"}
                </span>
              </div>
              <div className="text-white/60 text-[10px] mc-text-shadow text-left -mt-1">
          ...
          <div className="flex flex-col border-t border-white/10 pt-2 mt-2">
            <span className="text-[#FFFF55] text-sm mc-text-shadow font-bold">
              ● Skycoins: {skycoins}
            </span>
          </div>

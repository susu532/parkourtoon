...
export const DungeonDelverActionBar: React.FC<{ isMobile?: boolean }> = ({
  isMobile,
}) => {
  ...
  return (
    <div
      className={`absolute bottom-[44px] sm:bottom-16 md:bottom-20 lg:bottom-24 left-1/2 -translate-x-1/2 flex items-center justify-center gap-2 sm:gap-4 md:gap-12 pointer-events-none select-none mc-font w-full px-1 transform origin-bottom z-50 scale-[0.45] sm:scale-85 md:scale-100 ${isMobile ? "landscape:scale-[0.30] sm:landscape:scale-[0.30] md:landscape:scale-[0.35] lg:landscape:scale-[0.40]" : "landscape:scale-[0.45] sm:landscape:scale-85 md:landscape:scale-100"}`}
    >
      {/* Health */}

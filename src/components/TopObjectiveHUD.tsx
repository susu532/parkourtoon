...
export function TopObjectiveHUD() {
  ...
  const [waitingForNewMatchSequence, setWaitingForNewMatchSequence] =
    useState(false);
  ...
  useEffect(() => {
    if (gameState !== "playing") {
      ...
      if (
        timeLeft <= 1192 &&
        timeLeft >= 1188 &&
        !hasShownMatchStartText.current
      ) {
  ...
  let headerColorClass = inOvertime
  ...
  if (gameState === "endgame") {
    if (winningTeam === "draw") {
      ...
      headerColorClass =
        winningTeam.toLowerCase() === "red" ? "text-red-500" : "text-blue-400";

      if (playerTeam) {
        if (playerTeam === winningTeam) {
          resultText = "VICTORY!";
          resultColor =
            "text-yellow-400 font-extrabold shadow-[0_0_20px_#FFAA00] mc-text-shadow";
        } else {
          resultText = "DEFEAT";
          resultColor =
            "text-red-600 font-extrabold shadow-[0_0_20px_#FF0000] mc-text-shadow";
        }
      } else {
    ...
    <>
      ...
      <AnimatePresence>
        {showEndgameText && (
          ...
          >
            <h1
              className={`text-6xl md:text-8xl tracking-widest uppercase ${resultColor}`}
            >
              {resultText}
            </h1>
          </motion.div>

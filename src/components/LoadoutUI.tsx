...
export function LoadoutUI({ game, isMobile }: { game: Game | null, isMobile?: boolean }) {
  ...
  const selectLoadout = (type: 'bow' | 'painter' | 'washer') => {
    if (!game) return;
  ...
  if (showTutorial) {
    return (
      <div 
        className="absolute inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md pointer-events-auto flex-col gap-6"
        onPointerDown={e => e.stopPropagation()}
      >
        <img src="https://raw.githubusercontent.com/susu532/sounds/main/minecraft/1.png" alt="Tutorial" className="max-w-[90vw] max-h-[70vh] object-contain rounded-xl shadow-2xl border-4 border-[#555555]" />
        <button
          onClick={handleStartPlaying}
          className="bg-yellow-400 hover:bg-yellow-300 text-yellow-950 font-black px-8 py-4 text-2xl rounded-xl border-b-4 border-yellow-600 active:border-b-0 active:translate-y-1 transition-all shadow-[0_0_20px_rgba(250,204,21,0.6)] uppercase tracking-wider"
        >
          Start Playing!
        </button>
      </div>
    ...
    >
      <div className="mc-panel p-4 sm:p-6 flex flex-col items-center gap-4 sm:gap-6 animate-[slideUpFade_0.3s_ease-out_reverse] max-h-screen overflow-y-auto custom-scrollbar transform scale-[0.8] sm:scale-100 [@media(orientation:landscape)_and_(max-height:500px)]:scale-[0.65] [@media(orientation:landscape)_and_(max-height:400px)]:scale-[0.55]">
        ...
        <div className="flex flex-wrap items-center justify-center gap-6">
          <button
            onClick={(e) => { e.preventDefault(); selectLoadout('painter'); }}
          ...
          <button
            onClick={(e) => { e.preventDefault(); selectLoadout('bow'); }}
            className="flex flex-col items-center gap-4 bg-[#7B7B7B] border-[3px] border-t-white border-l-white border-b-[#555555] border-r-[#555555] p-6 hover:brightness-110 active:mc-button-shadow transition-all"
          >
            <div className="w-16 h-16 bg-zinc-800 flex items-center justify-center border-2 border-black/50 shadow-inner">
          ...
          <button
            onClick={(e) => { e.preventDefault(); selectLoadout('washer'); }}
            className="flex flex-col items-center gap-4 bg-[#7B7B7B] border-[3px] border-t-white border-l-white border-b-[#555555] border-r-[#555555] p-6 hover:brightness-110 active:mc-button-shadow transition-all"
          >
            <div className="w-16 h-16 bg-zinc-800 flex items-center justify-center border-2 border-black/50 shadow-inner">

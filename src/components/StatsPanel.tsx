...
export const StatsPanel: React.FC = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      const leaderboardKeybind =
        settingsManager.getSettings().keybinds.leaderboard || "Tab";
  ...
  const renderTable = (
    ...
    return (
      <div className="mb-4 w-full">
        <h3 className={`text-lg font-bold mb-2 ${colorClass}`}>
          {title} ({teamPlayers.length})
        </h3>
          ...
          <tbody>
            {teamPlayers.length === 0 ? (
              <tr>
                <td
                  colSpan={3}
                  className="py-2 px-2 text-center text-white/40 italic"
                >
                  Empty
                </td>
              </tr>
              ...
              teamPlayers.map((p) => {
                ...
                const nameColorClass =
                  isMe && isDungeonDelver ? "text-[#FFFF55]" : "";
                return (
                  <tr
                    key={p.id}
                  ...
                  >
                    <td
                      className={`py-1 px-2 truncate max-w-[120px] ${nameColorClass}`}
                    >
                      {p.name || "Unknown"}
                    </td>
                    <td className="py-1 px-2 text-center text-[#55FF55]">
                      {p.kills}
                    </td>
                    <td className="py-1 px-2 text-center text-[#FF5555]">
                      {p.deaths}
                    </td>
                  </tr>
                );

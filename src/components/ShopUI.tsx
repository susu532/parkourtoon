...
export const ShopUI = React.memo<ShopUIProps>(
  ({ npc, inventory, isOpen, onClose }) => {
    ...
    const [tradeOffer, setTradeOffer] = useState<(ItemStack | null)[]>(
      new Array(10).fill(null),
    );

    // Use a Ref to keep track of the current trade offer for high-frequency event handlers
    const tradeOfferRef = React.useRef<(ItemStack | null)[]>(
      new Array(10).fill(null),
    );
    React.useEffect(() => {
    ...
    const buyItems = useMemo(
      () => npc?.shopItems.filter((i) => i.action !== "sell") || [],
      [npc],
    );
    ...
    const playerCoins = useGameStore(
      (state) => state.skycoins[currentMode] ?? 500,
    );

    const getCurrencyCount = React.useCallback(
      (type: ItemType) => {
    ...
    const sellItemFromInventory = React.useCallback(
      (invIndex: number) => {
        ...
        if (remaining < item.count) {
          ...
          inventory.slots[invIndex] =
            remaining > 0 ? { ...item, count: remaining } : null;
    ...
    const removeTradeItem = React.useCallback(
      (index: number) => {
        const currentOffer = tradeOfferRef.current;
        const item = currentOffer[index];
        if (!item) return;

        const leftover = inventory.addItem(
          item.type,
          item.count,
          item.metadata,
        );
    ...
    const potentialGain = tradeOffer.reduce((sum, item) => {
      ...
      return (
        sum +
        (def
          ? Math.floor(item.count / (def.price || 1)) * (def.outputAmount || 0)
          : 0)
      );
    }, 0);
    ...
    return (
      <AnimatePresence>
        {isOpen && (
          <div
            className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 mc-font pointer-events-auto"
            onPointerDown={(e) => {
              e.stopPropagation();
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex gap-4 mt-1">
                        <span className="text-[#FFD700] font-bold text-sm flex items-center gap-1.5 bg-black/30 px-3 py-0.5 rounded-full border border-yellow-500/30">
                          <Coins size={14} /> {playerCoins.toLocaleString()}{" "}
                          Skycoins
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col md:flex-row flex-1 overflow-y-auto md:overflow-hidden">
                    ...
                    <div className="flex-1 md:border-r-[4px] border-b-[4px] md:border-b-0 border-[#222] flex flex-col p-4 md:p-6 bg-[#C6C6C6] min-h-[400px]">
                      <div className="flex items-center justify-between mb-4 px-1">
                        ...
                        <span className="text-[10px] text-[#373737] bg-white/50 px-2 py-0.5 rounded leading-none">
                          NPC Stock
                        </span>
                      </div>
                      ...
                      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                        <div className="grid grid-cols-5 gap-3">
                          {buyItems.map((item, i) => {
                            const canAfford =
                              getCurrencyCount(item.currency) >= item.price;
                            return (
                              <div
                                key={i}
                              ...
                              >
                                <Slot
                                  item={{
                                    type: item.type,
                                    count: item.outputAmount || 1,
                                  }}
                                  onHover={setHoveredItem}
                                  onClick={() => handleBuy(item)}
                                />
                                <div
                                  className={`text-[10px] font-bold flex items-center gap-0.5 ${canAfford ? "text-[#373737]" : "text-red-700"}`}
                                >
                                  {item.price}{" "}
                                  {item.currency === ItemType.SKYCOIN ? (
                                    <Coins size={10} />
                                  ) : (
                                    <span className="uppercase">
                                      {ITEM_NAMES[item.currency]?.substring(
                                        0,
                                        3,
                                      )}
                                    </span>
                      ...
                      <div className="mt-4 pt-4 border-t-2 border-[#8B8B8B] text-[11px] text-[#373737] leading-relaxed flex items-start gap-2 bg-white/20 p-3 rounded">
                        ...
                        />
                        <p>
                          Click items above to purchase directly into your
                          inventory. Some items require specific materials
                          instead of Skycoins.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex-1 flex flex-col p-6 bg-[#B0B0B0]">
                      <div className="flex items-center justify-between mb-4 px-1">
                        ...
                        <span className="text-[10px] text-[#373737] bg-white/50 px-2 py-0.5 rounded leading-none italic">
                          Sell Items Here
                        </span>
                      </div>
                      ...
                      <div className="mc-slot bg-[#8B8B8B]/50 p-4 mb-6 rounded-md">
                        <div className="flex justify-between items-center mb-2 px-1">
                          <span className="text-[10px] font-bold text-[#373737]/60 uppercase tracking-widest">
                            Trade Area
                          </span>
                          <button
                        ...
                        <div className="grid grid-cols-5 gap-2 mb-4">
                          {tradeOffer.map((item, i) => {
                            const isSellable =
                              item &&
                              sellItems.some((s) => s.type === item.type);
                            return (
                              <div
                                key={`trade-${i}`}
                                className={
                                  item && !isSellable ? "opacity-40" : ""
                                }
                              >
                                <Slot
                                  item={item}
                        ...
                        <div className="flex items-center justify-between pt-2 border-t-2 border-[#373737]/20">
                          <div>
                            <div className="text-[10px] uppercase font-bold text-[#373737] opacity-60">
                              Estimated Gain
                            </div>
                          ...
                          <button
                            ...
                            className={`px-6 h-10 mc-button-shadow font-bold flex items-center gap-2 transition-all ${
                              potentialGain > 0
                                ...
                                : "bg-gray-400 text-gray-600 grayscale cursor-not-allowed"
                            }`}
                          >
                        ...
                        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar bg-black/5 p-2 rounded">
                          <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-6 lg:grid-cols-8 gap-1.5 content-start">
                            {inventory.slots.slice(0, 36).map((slot, i) => {
                              const canSell =
                                slot &&
                                sellItems.some((s) => s.type === slot.type);
                              return (
                                <div
                                  key={`inv-${i}`}
                    ...
                    {hoveredItem && (
                      ...
                      >
                        <div
                          ...
                          style={{
                            color: hoveredItem.metadata?.rarity
                              ? RARITY_COLORS[
                                  hoveredItem.metadata.rarity as Rarity
                                ]
                              : "#FFF",
                          }}
                        >
                        ...
                        <div className="text-gray-400 text-xs leading-relaxed mb-4">
                          {hoveredItem.metadata?.description ||
                            "A standard item found in the world."}
                        </div>

...
export const DungeonDelverSidebar: React.FC<DungeonDelverSidebarProps> = ({
  isMobile,
}) => {
  ...
  return (
    ...
    >
      <div className="bg-black/60 p-3 md:p-4 border-l-4 border-red-500 text-white text-sm md:text-base shadow-2xl min-w-[160px] md:min-w-[200px]">
        <div className="text-red-500 font-bold mb-1 text-center uppercase tracking-[0.1em] text-lg mc-text-shadow">
          DUNGEON DELVER
        </div>
        ...
        <div className="space-y-2">
          <div className="border-t border-white/10 pt-3 mt-3">
            <span className="text-red-400 text-sm mc-text-shadow font-bold block mb-2 uppercase tracking-wide">
              Top Slayers
            </span>
            {players.length === 0 && (
              <div className="text-white/40 text-xs mc-text-shadow italic">
                No slayers yet...
              </div>
            )}
            {players.map((p, index) => {
              ...
              return (
                <div
                  key={p.id}
                  className="flex justify-between items-center bg-black/40 px-2 py-1.5 mb-1 border-l-2 border-red-500/50"
                >
                  <div className="flex items-center gap-2">
                    <span className="text-white/50 text-xs">{index + 1}.</span>
                    <span
                      className={`truncate max-w-[90px] mc-text-shadow text-sm ${isMe ? "text-[#FFFF55]" : "text-white"}`}
                    >
                      {p.name || "Unknown"}
                    </span>
                  </div>
                  <span className="text-red-400 font-bold mc-text-shadow">
                    {p.kills}
                  </span>
                </div>
              );

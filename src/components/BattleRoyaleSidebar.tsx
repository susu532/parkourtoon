...
export const BattleRoyaleSidebar: React.FC<BattleRoyaleSidebarProps> = ({
  isMobile,
}) => {
      ...
      for (let x = -400; x <= 400; x += gridSpacing) {
        for (let z = -400; z <= 400; z += gridSpacing) {
          ...
          if (plotType === "PARK") {
            ...
            ctx.fillRect(
              screenX + (offset - 25) * scale,
              screenZ + (offset - 25) * scale,
              50 * scale,
              50 * scale,
            );
            ...
            ctx.fillRect(
              screenX + (offset - 20) * scale,
              screenZ + (offset - 15) * scale,
              40 * scale,
              30 * scale,
            );
            ...
            ctx.fillRect(
              screenX + (offset - 35) * scale,
              screenZ + (offset - 20) * scale,
              70 * scale,
              40 * scale,
            );
            ...
            ctx.beginPath();
            ctx.arc(
              screenX + offset * scale,
              screenZ + offset * scale,
              38 * scale,
              0,
              Math.PI * 2,
            );
            ...
            ctx.fillRect(
              screenX + (offset - 5) * scale,
              screenZ + (offset - 15) * scale,
              10 * scale,
              30 * scale,
            );
            ctx.fillRect(
              screenX + (offset - 15) * scale,
              screenZ + (offset - 5) * scale,
              30 * scale,
              10 * scale,
            );
          }
  ...
  return (
    ...
    >
      ...
      <div className="bg-black/60  p-3 md:p-4 border-l-4 border-[#FFAA00] text-white text-sm md:text-base shadow-2xl min-w-[160px] md:min-w-[200px]">
        <div className="text-[#FFAA00] font-bold mb-1 text-center uppercase tracking-[0.1em] text-lg mc-text-shadow">
          BATTLE ROYALE
        </div>
        ...
        <div className="space-y-4">
          <div className="flex flex-col">
            ...
            <span className="text-[#55FFFF] font-bold text-3xl mc-text-shadow">
              {playersAlive}
            </span>
          </div>

          <div className="flex flex-col border-t border-white/10 pt-2 mt-2">
            <span className="text-[#FFAA00] text-sm mc-text-shadow font-bold">
              Kills: 0
            </span>
          </div>

          <div className="flex flex-col border-t border-white/10 pt-2 mt-2">
            <span className="text-gray-300 text-sm mc-text-shadow overflow-hidden whitespace-nowrap overflow-ellipsis">
              Zone shrinking in...
            </span>

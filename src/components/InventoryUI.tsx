...
export const InventoryUI = React.memo<InventoryUIProps>(
  ({ inventory, isOpen, onClose, onDropItem }) => {
    const [craftingGrid, setCraftingGrid] = useState<(ItemStack | null)[]>(
      new Array(4).fill(null),
    );
    const [craftingResult, setCraftingResult] = useState<ItemStack | null>(
      null,
    );
    ...
    const skycoins = useGameStore(
      (state) => state.skycoins[currentMode] ?? 500,
    );
    ...
    useEffect(() => {
      if (!isOpen) {
        ...
        if (heldItemRefState.current)
          itemsToReturn.push({ ...heldItemRefState.current });
    ...
    const handleSlotInteraction = useStableCallback(
      (
        type: "inv" | "grid",
        index: number,
            ...
            if (button === 0) {
              ...
              if (!dragState.visitedSlots.has(slotId)) {
                ...
                if (!target) {
                  ...
                  setDragState((prev) => ({
                    ...prev,
                    visitedSlots: new Set(prev.visitedSlots).add(slotId),
                  }));
                }
              }
            } else if (button === 2) {
                ...
                if (!target || target.type === heldItem.type) {
                  if (type === "inv") {
                    if (!inventory.slots[actualIndex]) {
                      ...
                      getMaxStack(heldItem.type)
                    ) {
                      inventory.slots[actualIndex] = {
                        ...inventory.slots[actualIndex]!,
                        count: inventory.slots[actualIndex]!.count + 1,
                      };
                    } else {
                      return; // Full
                    }
                    ...
                    setCraftingGrid((prev) => {
                      ...
                      if (!next[index]) {
                        ...
                        next[index]!.count < getMaxStack(heldItem.type)
                      ) {
                        next[index] = {
                          ...next[index]!,
        ...
        const actualIndex = isHotbar ? index : index + 9;
        const target =
          type === "inv" ? inventory.slots[actualIndex] : craftingGrid[index];
        if (heldItem && (!target || target.type === heldItem.type)) {
          setDragState({
            isDragging: true,
            button,
            visitedSlots: new Set([`${type}-${actualIndex}`]),
          });
        } else {
          setDragState({
            isDragging: false,
    ...
    const handleSlotClick = useStableCallback(
      (
        index: number,
        isHotbar: boolean = false,
        ...
        if (isShiftClick) {
          if (slotItem) {
            ...
            for (let i = targetRange[0]; i <= targetRange[1]; i++) {
              ...
              if (
                target &&
                target.type === slotItem.type &&
                target.count < getMaxStack(slotItem.type)
              ) {
                const canAdd = Math.min(
                  remaining,
                  getMaxStack(slotItem.type) - target.count,
                );
                inventory.slots[i] = {
                  ...target,
                  count: target.count + canAdd,
                };
                remaining -= canAdd;
              }
          ...
          if (!heldItem && slotItem) {
            ...
            inventory.slots[actualIndex] =
              remaining > 0 ? { ...slotItem, count: remaining } : null;
          ...
          if (heldItem && !slotItem) {
            ...
            if (heldItem.type === slotItem.type) {
              const canAdd = Math.min(
                heldItem.count,
                getMaxStack(slotItem.type) - slotItem.count,
              );
              inventory.slots[actualIndex] = {
                ...slotItem,
                count: slotItem.count + canAdd,
              };
    ...
    const handleCraftingClick = useStableCallback(
      (index: number, isRightClick: boolean = false) => {
        audioManager.play("click", 0.5, 1.0 + Math.random() * 0.2);
        ...
        if (isRightClick) {
          if (!heldItem && slotItem) {
            ...
            heldItem &&
            (!slotItem || slotItem.type === heldItem.type)
          ) {
            // Place 1
            setCraftingGrid((prev) => {
              const next = [...prev];
    ...
    const handleResultClick = useStableCallback(
      (isShiftClick: boolean = false) => {
        ...
        if (isShiftClick) {
          ...
          while (true) {
            ...
            for (let i = 0; i < 36; i++) {
              ...
              if (
                ...
                slot.count < getMaxStack(result.type)
              ) {
                remaining -= Math.min(
                  remaining,
                  getMaxStack(result.type) - slot.count,
                );
              }
              if (remaining <= 0) break;
            }
            ...
            craftedAny = true;
            currentGrid = currentGrid.map((s) =>
              s ? (s.count > 1 ? { ...s, count: s.count - 1 } : null) : null,
            );
          }
          ...
          if (
            craftingResult &&
            (!heldItem ||
              (heldItem.type === craftingResult.type &&
                heldItem.count + craftingResult.count <=
                  getMaxStack(craftingResult.type)))
          ) {
            if (heldItem) {
              setHeldItem({
                ...heldItem,
                count: heldItem.count + craftingResult.count,
              });
            } else {
    ...
    const handleDoubleClick = useStableCallback(() => {
      ...
      for (let i = 0; i < 37; i++) {
        ...
        if (slot && slot.type === heldItem.type) {
          ...
          inventory.slots[i] =
            newCount > 0 ? { ...slot, count: newCount } : null;
          gathered += take;
      ...
      if (gathered > 0) {
        setHeldItem((prev) =>
          prev ? { ...prev, count: prev.count + gathered } : null,
        );
        useGameStore.getState().incrementInventoryVersion();
      }
    });
    ...
    return (
      ...
      >
        <div
          ...
          style={{
            transform: isMobileLandscape ? "none" : `scale(${containerScale})`,
          }}
        >
          <div
            ...
            {isMobileLandscape ? (
              ...
              >
                <div className="flex flex-wrap items-center gap-2 md:gap-4 mb-4 border-b-2 border-[#373737]/30 pb-2">
                  ...
                  <button
                    onClick={onClose}
                    className="ml-auto text-[#373737] hover:text-red-600 font-bold px-2 text-xl"
                  >
                    ✕
                  </button>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 sm:gap-8 mb-4 sm:mb-8">
                  <div className="flex sm:flex-col gap-4">
                    ...
                    <div className="flex flex-col items-center gap-1 justify-center">
                      <span className="text-[8px] sm:text-[10px] uppercase opacity-60 font-bold">
                        Off-hand
                      </span>
                      <Slot
                        item={inventory.slots[Inventory.OFF_HAND_SLOT]}
                        onClick={(item, button, isShift) =>
        ...
        {hoveredItem && !heldItem && !isMobileLandscape && (
          ...
          >
            <div
              ...
              style={{
                color:
                  RARITY_COLORS[hoveredItem.metadata?.rarity || Rarity.COMMON],
              }}
            >
            ...
            {hoveredItem.metadata?.stats && (
              <div className="space-y-0.5 mb-2 border-b border-white/10 pb-2">
                {Object.entries(hoveredItem.metadata.stats).map(
                  ([stat, value]) => {
                    const isBaseStat =
                      stat === "damage" ||
                      stat === "health" ||
                    ...
                    return (
                      <div
                        key={stat}
                        className="flex justify-between mc-text-shadow"
                      >
                        <span className="text-[#AAAAAA] capitalize">
            ...
            {hoveredItem.metadata?.ability && (
              <div className="mb-2 border-t border-white/10 pt-2">
                <div className="text-[#FFFF55] font-bold uppercase text-xs mc-text-shadow">
                  Ability: {hoveredItem.metadata.ability.name}
                </div>
                <div className="text-[#AAAAAA] text-xs leading-tight mc-text-shadow">
                  {hoveredItem.metadata.ability.description}
                </div>
                {hoveredItem.metadata.ability.manaCost && (
                  <div className="text-[#55FFFF] text-[10px] mt-1 mc-text-shadow">
                    Mana Cost: {hoveredItem.metadata.ability.manaCost}
                  </div>
                )}
              </div>
            )}

            {hoveredItem.metadata?.durability !== undefined &&
              hoveredItem.metadata?.maxDurability !== undefined && (
                <div className="text-[#AAAAAA] text-xs mt-1 border-t border-white/10 pt-1 mc-text-shadow">
                  Durability: {hoveredItem.metadata.durability} /{" "}
                  {hoveredItem.metadata.maxDurability}
                </div>
              )}
        ...
        {heldItem && (
          ...
          >
            ...
            {heldItem?.metadata?.durability !== undefined &&
              heldItem?.metadata?.maxDurability !== undefined && (
                <div className="absolute bottom-0 left-0 w-full h-1 bg-black/50 pointer-events-none">
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor:
                        heldItem.metadata.durability /
                          heldItem.metadata.maxDurability >
                        0.5
                          ? "#00FF00"
                          : heldItem.metadata.durability /
                                heldItem.metadata.maxDurability >

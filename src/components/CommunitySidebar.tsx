...
export const CommunitySidebar: React.FC = () => {
  ...
  const removeFriendRequest = useGameStore(
    ...
    const initCG = async () => {
      try {
        if ((window as any).CrazyGames?.SDK) {
          ...
          if ((window as any).CrazyGames.SDK.data) {
            const savedStatus = await (
              window as any
            ).CrazyGames.SDK.data.getItem("starplex_user_status");
  ...
  const getDynamicFriendInfo = (friend: FriendItem) => {
    ...
    const activePlayer = activeSessionPlayers.find(
      (p: any) =>
        p && p.name && p.name.toUpperCase() === friend.name.toUpperCase(),
    );
  ...
  const handleAddFriendSubmit = (e: React.FormEvent) => {
    ...
    useGameStore
      ...
      .addChatMessage(
        "System",
        `§eFriend request sent to ${newFriendName.trim()}`,
      );
    setNewFriendName("");
  };
  ...
  const handleAcceptPartyInvite = (req: {
    sourceId: string;
    sourceName: string;
    server: string;
  }) => {
  ...
  const lobbyPlayers = activeSessionPlayers
    ...
    .filter(
      (p: any) =>
        !friends.some((f) => f.name.toUpperCase() === p.name.toUpperCase()),
    );
        ...
        <div className="bg-[#8B8B8B] border-t-2 border-l-2 border-white border-b-2 border-r-2 border-[#555555] p-2 sm:p-3 space-y-1.5 sm:space-y-3 shrink-0">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[9px] text-green-300 font-bold tracking-wider pt-0.5 uppercase">
                ● Profile Account
              </p>
            </div>
          </div>
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-black/60 uppercase block">
              Edit Username
            </label>
            <input
              type="text"
      ...
      {activeTab === "social" && (
        <div className="bg-[#8B8B8B] border-t-2 border-l-2 border-white border-b-2 border-r-2 border-[#555555] p-2 sm:p-3 flex flex-col gap-1 sm:gap-2 flex-1 min-h-0 overflow-hidden">
          ...
          {(friendRequests.length > 0 || partyInvites.length > 0) && (
            <div className="flex-none mb-1 border-b-2 border-black/15 pb-2">
              {friendRequests.map((req, i) => (
                <div
                  key={`fr-${i}`}
                  className="bg-yellow-600/30 border border-yellow-500/50 p-1.5 flex justify-between items-center mb-1"
                >
                  <span className="text-[10px] font-bold text-white tracking-wide uppercase">
                    <span className="text-yellow-300">{req.sourceName}</span>{" "}
                    sent a friend request
                  </span>
                  <div className="flex gap-1">
                    <button
                      onClick={() => handleAcceptFriendRequest(req)}
                      className="bg-green-600 hover:bg-green-500 p-1 border border-black/50 text-white"
                    >
                      <Check className="w-3 h-3" />
                    </button>
                    <button
                      onClick={() => removeFriendRequest(req.sourceId)}
                      className="bg-red-600 hover:bg-red-500 p-1 border border-black/50 text-white"
                    >
                      <X className="w-3 h-3" />
                    </button>
                  </div>
              ...
              {partyInvites.map((req, i) => (
                <div
                  key={`pi-${i}`}
                  className="bg-blue-600/30 border border-blue-500/50 p-1.5 flex justify-between items-center mb-1"
                >
                  <span className="text-[10px] font-bold text-white tracking-wide uppercase">
                    <span className="text-blue-300">{req.sourceName}</span>{" "}
                    invited you to Party
                  </span>
                  <div className="flex gap-1">
                    <button
                      onClick={() => handleAcceptPartyInvite(req)}
                      className="bg-green-600 hover:bg-green-500 p-1 border border-black/50 text-white"
                    >
                      <Check className="w-3 h-3" />
                    </button>
                    <button
                      onClick={() => removePartyInvite(req.sourceId)}
                      className="bg-red-600 hover:bg-red-500 p-1 border border-black/50 text-white"
                    >
                      <X className="w-3 h-3" />
                    </button>
          ...
          <div className="flex-1 overflow-y-auto space-y-1.5 pr-1 py-1 custom-scrollbar min-h-0">
            {friends.map((friend) => {
              ...
              return (
                <div
                  key={friend.id}
                  className="flex items-center justify-between bg-black/20 p-1.5 border border-black/15 hover:bg-black/30 transition-colors"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0 flex flex-col items-start text-left">
                      <span className="text-white text-xs font-bold truncate leading-none mb-0.5 uppercase text-left">
                        {friend.name}
                      </span>
                      <span
                        className={`text-[9px] truncate tracking-wide text-left ${info.online ? "text-green-300 font-bold" : "text-white/40"}`}
                      >
                        {info.status}
                      </span>
                    </div>
                    ...
                    <button
                      onClick={() => {
                        ...
                        useGameStore
                          ...
                          .addChatMessage(
                            "System",
                            `§eParty invite sent to ${friend.name} (if online).`,
                          );
                      }}
            ...
            {friends.length === 0 && (
              <div className="text-white/40 text-[10px] italic text-center py-4">
                No friends added...
              </div>
            )}
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-1.5 pr-1 py-1 custom-scrollbar min-h-0">
            {lobbyPlayers.map((p: any) => {
              ...
              return (
                <div
                  key={p.id}
                  className="flex items-center justify-between bg-black/10 p-1.5 border border-black/15 hover:bg-black/20 transition-colors"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="w-5 h-5 bg-[#C6895C] border border-black/30 shrink-0">
                      <svg
                  ...
                  <button
                    onClick={() => {
                      ...
                      useGameStore
                        ...
                        .addChatMessage(
                          "System",
                          `§eFriend request sent to ${p.name}`,
                        );
                    }}
            ...
            {lobbyPlayers.length === 0 && (
              <div className="text-white/30 text-[9px] italic text-center py-2">
                No other players in this lobby...
              </div>
            )}
          </div>
          ...
          <form
            ...
            className="mt-2 flex gap-1 pt-1.5 border-t border-black/15 shrink-0"
          >
            <input
              type="text"
              value={newFriendName}

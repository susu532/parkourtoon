...
import {
  getTextureAtlasDataUrl,
  getBlockUVs,
  isFlatItem,
  isPlant,
} from "../../game/TextureAtlas";
  ...
  ({
    ...
    return (
      <div
        ...
        style={{
          borderWidth:
            item?.metadata?.rarity && item.metadata.rarity !== Rarity.COMMON
              ? "3px"
              : "2px",
          borderColor: item?.metadata?.rarity
            ? RARITY_COLORS[item.metadata.rarity]
            : undefined,
      ...
      >
        ...
        {(() => {
          ...
          const isTool =
            item?.type &&
            ((item.type >= 436 && item.type <= 440) ||
              (item.type >= 446 && item.type <= 455));
          const serverName =
          ...
          const shouldShow =
            ...
            item?.metadata?.maxDurability !== undefined &&
            !isSword &&
            !(isTool && isSkyCastles);
...
export const ItemIcon = React.memo<{ item: ItemStack }>(({ item }) => {
  ...
  if (item.type === ItemType.MINION) {
    return (
      <div className="relative w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center select-none bg-gradient-to-br from-[#FFFF55] to-[#DADA44] rounded-sm border-2 border-black/40 shadow-lg">
        <div className="text-[6px] sm:text-[9px] font-black text-black text-center leading-tight uppercase tracking-tighter">
          MINION
        </div>
        {item.count > 1 && (
    ...
    return (
      <div className="relative w-[20px] h-[20px] sm:w-[24px] sm:h-[24px] flex items-center justify-center select-none bg-zinc-900 rounded-sm border border-black/80 shadow-md">
        <div className="flex flex-col items-center">
          ...
          <div
            className={`w-[8px] h-[10px] ${isWasher ? "bg-[#3889f0]" : "bg-[#3d1c04]"} rounded-[2px] shadow-[inset_0_2px_4px_rgba(0,0,0,0.5)] border ${isWasher ? "border-[#1e5dab]" : "border-[#261001]"}`}
          ></div>
        </div>
        {item.count > 1 && (
          <span className="absolute -bottom-1 -right-1 text-[10px] sm:text-[12px] font-bold text-white drop-shadow-[1.5px_1.5px_0_rgba(0,0,0,1)] pointer-events-none z-10">
  ...
  if (!flat) {
    ...
    return (
      <div
        className="relative w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center select-none"
  ...
  const isWeaponOrToolOrBow =
    item?.type &&
    ((item.type >= 436 && item.type <= 455) || // tools and swords
      item.type === 461 ||
      item.type === 54); // bow, AOTE

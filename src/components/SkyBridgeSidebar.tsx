...
export const SkyBridgeSidebar: React.FC<SkyBridgeSidebarProps> = ({
  isMobile,
}) => {
  ...
  const skills =
    playerSkills && Object.keys(playerSkills).length > 0
      ? playerSkills
  ...
  return (
    ...
    >
      ...
      <div className="bg-black/60  p-3 md:p-4 border-l-4 border-[#FFFF55] text-white text-sm md:text-base shadow-2xl min-w-[160px] md:min-w-[200px]">
        <div className="text-[#FFFF55] font-bold mb-1 text-center uppercase tracking-[0.1em] text-lg mc-text-shadow">
          skybridge
        </div>
        ...
        <div className="space-y-2">
          <div className="flex flex-col">
            <span className="text-white text-sm mc-text-shadow">
              Level: <span className="text-[#55FFFF] font-bold">1</span>
            </span>
          ...
          <div className="pt-2 space-y-1 border-t border-white/10 text-sm mc-text-shadow">
            ...
            <div className="flex justify-between gap-4">
              ...
              <span className="text-[#5555FF] font-bold">
                {stats.critChance}%
              </span>
            </div>
            <div className="flex justify-between gap-4">
        ...
        <div className="mt-4 text-[#55FF55] font-bold mb-2 border-t border-white/10 pt-3 uppercase tracking-widest text-sm mc-text-shadow">
          Skills
        </div>
        <div className="space-y-2 text-sm mc-text-shadow">
          {(Object.entries(skills) as [string, any][]).map(
            ([name, progress]) => (
              <div key={name} className="flex flex-col">
                <div className="flex justify-between items-baseline">
                  <span className="text-white/80">{name}</span>
                  <span className="text-[#FFFF55] text-xs">
                    Lvl {progress.level}
                  </span>
                </div>

...
export const ChestUI = React.memo<{
  ...
  useEffect(() => {
    if (!isOpen) {
      if (heldItemRefState.current) {
        const remaining = playerInventory.addItem(
          heldItemRefState.current.type,
          heldItemRefState.current.count,
          heldItemRefState.current.metadata,
        );
  ...
  const handleSlotAction = useStableCallback(
    (
      ...
      let targetInventory =
        sourceGroup === "chest" ? chestInventory : playerInventory;
      const actualIndex =
        sourceGroup === "inventory" ? slotIndex + 9 : slotIndex;
        ...
        if (heldItemRefState.current) {
          if (button === 0) {
            ...
            if (!dragState.visitedSlots.has(slotId)) {
              ...
              if (!target) {
                targetInventory.slots[actualIndex] = {
                  ...heldItemRefState.current,
                  count: 1,
                };
                const newHeld = {
                  ...heldItemRefState.current,
                  count: heldItemRefState.current.count - 1,
                };
            ...
            if (!dragState.visitedSlots.has(slotId)) {
              ...
              if (!target || target.type === heldItemRefState.current.type) {
                if (!target) {
                  ...
                  target.count < getMaxStack(heldItemRefState.current.type)
                ) {
                  targetInventory.slots[actualIndex] = {
                    ...target,
      ...
      if (
        heldItemRefState.current &&
        (!slotItem || slotItem.type === heldItemRefState.current.type)
      ) {
        setDragState({
          isDragging: true,
          button,
          visitedSlots: new Set([slotId]),
        });
      } else {
        setDragState({
          isDragging: false,
      ...
      if (isShift && slotItem && !heldItemRefState.current) {
        ...
        const destinationInv =
          sourceGroup === "chest" ? playerInventory : chestInventory;
        const remaining = destinationInv.addItem(
          slotItem.type,
          slotItem.count,
          slotItem.metadata,
        );
      ...
      if (!pendingHeld && slotItem) {
        if (button === 0) {
          ...
          targetInventory.slots[actualIndex] =
            remainder > 0 ? { ...slotItem, count: remainder } : null;
          audioManager.play("click", 0.5, 1.2);
        }
      } else if (pendingHeld) {
        if (!slotItem) {
          ...
          if (slotItem.type === pendingHeld.type) {
            if (button === 0) {
              ...
              button === 2 &&
              slotItem.count < getMaxStack(slotItem.type)
            ) {
              targetInventory.slots[actualIndex] = {
                ...slotItem,
    ...
    for (let i = 0; i < 36; i++) {
      ...
      if (slot && slot.type === pendingHeld.type) {
        ...
        playerInventory.slots[i] =
          slot.count - take > 0 ? { ...slot, count: slot.count - take } : null;
        gathered += take;
  ...
  const renderTooltip = () => {
    ...
    return (
      ...
      >
        <div
          className="font-bold mc-text-shadow leading-tight mb-1"
          style={{ color: "#55FFFF" }}
        >
          ...
          <AnimatePresence>
            ...
            >
              ...
              <div className="flex justify-between items-center mb-2">
                <h2 className="text-xl font-bold mc-text-shadow text-white uppercase tracking-wide">
                  Chest
                </h2>
                <button
                  onClick={onClose}
                  className="mc-panel px-3 py-1 text-white hover:bg-[#8B8B8B] mc-text-shadow active:bg-[#555555]"
                >
                  X
                </button>
              </div>
              ...
              <div className="flex flex-col gap-6 w-full">
                  ...
                  <div className="grid grid-cols-9 gap-[2px]">
                    {chestInventory.slots.slice(0, 27).map((item, i) => (
                      <Slot
                        key={`chest-${i}`}
                        item={item}
                        onClick={(_, button, isShift, isEnter) =>
                          handleSlotAction(
                            item,
                            "chest",
                            i,
                ...
                <div className="w-full flex justify-center">
                  <div className="text-gray-300 font-bold uppercase tracking-widest text-sm mc-text-shadow">
                    Inventory
                  </div>
                </div>

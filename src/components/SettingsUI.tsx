...
const translations: Record<string, Record<string, string>> = {
  en: {
    ...
    performanceDesc:
      "Reduces render distance and disables heavy effects for smoother gameplay",
  ...
  es: {
    ...
    performanceDesc:
      "Reduce la distancia de renderizado y desactiva efectos pesados",
  ...
  fr: {
    ...
    performanceDesc:
      "Réduit la distance d'affichage et désactive les effets pour plus de fluidité",
    premiumShaders: "Shaders Premium",
  ...
  de: {
    ...
    performanceDesc:
      "Reduziert die Sichtweite und deaktiviert komplexe Effekte für mehr FPS",
...
export const SettingsUI: React.FC<SettingsUIProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const isDuplicate = Object.entries(settings.keybinds).find(
        ([key, code]) => code === e.code && key !== rebindingKey,
      );
      if (isDuplicate) {
            ...
            <div className="p-6 h-[55vh] md:h-[60vh] overflow-y-auto custom-scrollbar space-y-8 bg-[#C6C6C6] select-none text-left">
              ...
              <section className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div
                    className={`space-y-2 ${settings.performanceMode ? "opacity-50 pointer-events-none" : ""}`}
                  >
                    <div className="flex justify-between">
                      <label className="text-sm font-bold text-[#555555] uppercase font-sans">
                        {t.renderDistance}
                      </label>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleChange("renderDistance", parseInt(e.target.value))
                      }
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      value={settings.fov}
                      onChange={(e) =>
                        handleChange("fov", parseInt(e.target.value))
                      }
                      className="w-full h-4 bg-[#8B8B8B] appearance-none cursor-pointer border-2 border-black/20"
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between p-3 bg-[#A0A0A0] border-2 border-black/20 md:col-span-2">
                    <div className="flex flex-col">
                      <div className="flex items-center gap-2">
                        ...
                        <label className="text-sm font-bold text-[#555555] uppercase">
                          {t.performanceMode}
                        </label>
                      </div>
                    ...
                    >
                      <div
                        className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${settings.performanceMode ? "left-7" : "left-1"}`}
                      />
                    </button>
                  </div>

                  <div
                    className={`flex items-center justify-between p-3 bg-[#A0A0A0] border-2 border-black/20 md:col-span-2 ${settings.performanceMode ? "opacity-50 pointer-events-none" : ""}`}
                  >
                    <div className="flex flex-col">
                      <label className="text-sm font-bold text-[#555555] uppercase">
                        {t.premiumShaders}
                    ...
                    <button
                      disabled={settings.performanceMode}
                      onClick={() =>
                        handleChange("premiumShaders", !settings.premiumShaders)
                      }
                      className={`w-12 h-6 rounded-full transition-colors relative shrink-0 ${settings.premiumShaders ? "bg-green-500" : "bg-[#555555]"}`}
                    >
                      <div
                        className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${settings.premiumShaders ? "left-7" : "left-1"}`}
                      />
                    </button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="text-sm font-bold text-[#555555]">
                        {Math.round(settings.sensitivity * 10000)}
                      </span>
                    </div>
                    <input
                      type="range"
                  ...
                  <div className="flex items-center justify-between p-3 bg-[#A0A0A0] border-2 border-black/20">
                    ...
                    <button
                      onClick={() =>
                        handleChange("invertMouse", !settings.invertMouse)
                      }
                      className={`w-12 h-6 rounded-full transition-colors relative shrink-0 ${settings.invertMouse ? "bg-green-500" : "bg-[#555555]"}`}
                    >
                      <div
                        className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${settings.invertMouse ? "left-7" : "left-1"}`}
                      />
                    </button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {Object.entries(settings.keybinds)
                    ...
                    .map(([name, code]) => (
                      <div
                        key={name}
                        className="flex items-center justify-between p-2 bg-[#A0A0A0] border-2 border-black/10"
                      >
              ...
              <section className="space-y-4">
                ...
                <div className="flex items-center justify-between p-3 bg-[#A0A0A0] border-2 border-black/20">
                  ...
                  <button
                    onClick={() =>
                      handleChange("showDebug", !settings.showDebug)
                    }
                    className={`w-12 h-6 rounded-full transition-colors relative shrink-0 ${settings.showDebug ? "bg-green-500" : "bg-[#555555]"}`}
                  >
                    <div
                      className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${settings.showDebug ? "left-7" : "left-1"}`}
                    />
                  </button>
                </div>
              </section>

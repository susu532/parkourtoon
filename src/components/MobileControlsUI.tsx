      ...
      for (let i = 0; i < e.changedTouches.length; i++) {
        ...
        if (touch.identifier === joystickTouchId.current) {
          ...
          if (origin) {
            ...
            // To update Sprint icon visibility and rotation gracefully, we can just let game logic handle it, but for UI we may still need a rare trigger.
            // But we actually only care about joystick transform. We can skip setJoystick.
    ...
    const handleTouchEnd = (e: TouchEvent) => {
      for (let i = 0; i < e.changedTouches.length; i++) {
        ...
        if (touch.identifier === joystickTouchId.current) {
          ...
          // Joystick thumb will unmount anyway when origin is null
        ...
        {joystickOrigin && (
          ...
          >
            ...
            >
              <Navigation
                size={20}
                className={`text-white drop-shadow-md opacity-100 transition-opacity`}
              />
            </div>
          </div>
        )}
